...
export async function run(containers, options = {}) {
    ...
    console.log(`\n${'='.repeat(80)}`);
    console.log(`🔍 [INTEGRATIONSTATUS-MONTH] Starting query for time window: ${window.startSec} to ${window.endSec}`);
    ...
    try {
        ...
        if (result.resources.length === 0) {
            ...
            statuses.forEach(intStatus => {
                // Total
                metrics.push({
                    metric: `${this.metricBase},integration_status=${intStatus},country=total,window=month`,
                    value: 0,
                    timestamp
                });
                ...
                ['au', 'nz'].forEach(country => {
                    metrics.push({
                        metric: `${this.metricBase},integration_status=${intStatus},country=${country},window=month`,
                        value: 0,
                        timestamp
                    });
                });
                ...
                contractStatuses.forEach(cStatus => {
                    metrics.push({
                        metric: `${this.metricBase},integration_status=${intStatus},contract_status=${cStatus},country=total,window=month`,
                        value: 0,
                        timestamp
                    });
                });
        ...
        Object.keys(statusCounts).forEach(intStatus => {
            ...
            // Total metrics
            metrics.push({
                metric: `${this.metricBase},integration_status=${normalizedStatus},country=total,window=month`,
            ...
            ['au', 'nz'].forEach(country => {
                metrics.push({
                    metric: `${this.metricBase},integration_status=${normalizedStatus},country=${country},window=month`,
            ...
            contractStatuses.forEach(cStatus => {
                ...
                metrics.push({
                    metric: `${this.metricBase},integration_status=${normalizedStatus},contract_status=${cStatus},country=total,window=month`,
                    value: count,
                    timestamp
                });
            });

// src/queries/month/monthprocessed.js
// All contract processed queries for month
...
export const queries = [
    {
        name: "contractprocessed_month_by_status",
        metricBase: "custom.dashboard.contractprocessed.month.by_status",
        run: async (containers, win) => {
            console.log(`🔍 [CONTRACTPROCESSED-MONTH] Starting query for time window: ${win.startSec} to ${win.endSec}`);
            ...
            console.log(`🔍 [CONTRACTPROCESSED-MONTH] Querying contract table for documents in time window...`);
            const { resources: contractRows } = await statusC.items.query(qContracts).fetchAll();
            console.log(`✅ [CONTRACTPROCESSED-MONTH] Found ${contractRows?.length || 0} contracts in time window`);

            if (!contractRows || contractRows.length === 0) {
                console.log(`⚠️ [CONTRACTPROCESSED-MONTH] No contracts found, returning zero metrics`);
                return generateProcessedCumulativeMetricsFromData(new Map(), "month");
            }
            ...
            const cleanContractRows = contractRows.filter((d, i) => {
                ...
                if (!ok) {
                    console.warn(`[CONTRACTPROCESSED-MONTH] Skipping row at index ${i} due to invalid batchId:`, {
            ...
            if (cleanContractRows.length === 0) {
                console.log(`⚠️ [CONTRACTPROCESSED-MONTH] No valid rows with batchId; returning zero metrics`);
                return generateProcessedCumulativeMetricsFromData(new Map(), "month");
            }
            ...
            console.log(`🔍 [CONTRACTPROCESSED-MONTH] Processing ${batchIds.length} batchIds for processed analysis...`);
            ...
            for (let i = 0; i < batchIds.length; i += chunkSize) {
                ...
                console.log(`🔍 [CONTRACTPROCESSED-MONTH] Looking up entity data for chunk ${Math.floor(i/chunkSize) + 1}...`);
                ...
                for (const entity of (entities || [])) {
                    ...
                    console.log(`✅ [CONTRACTPROCESSED-MONTH] Entity data for ${entity.batchId}: country=${entity.countryCode}, status=${entity.contractStatus}, contractId=${entity.contractId}`);
                }
            }

            console.log(`✅ [CONTRACTPROCESSED-MONTH] Entity data lookup complete. Found ${entityData.size} entities`);

            // Step 3: Build a map of contractId -> most recent contract document
            console.log(`🔍 [CONTRACTPROCESSED-MONTH] Finding most recent upload for each contract...`);
            ...
            for (const contractRow of cleanContractRows) {
                ...
                if (!entityBatchId) {
                    console.warn(`[CONTRACTPROCESSED-MONTH] Derived empty entityBatchId from batchId='${batchId}', skipping`);
                    continue;
                }
                ...
                if (entity && entity.contractId) {
                    ...
                    if (!existing || contractRow.timestamp > existing.contractRow.timestamp) {
                        ...
                        if (existing) {
                            console.log(`🔄 [CONTRACTPROCESSED-MONTH] Replaced older upload for contract ${contractId} (old ts: ${existing.contractRow.timestamp}, new ts: ${contractRow.timestamp})`);
                        }
                    } else {
                        console.log(`⏭️ [CONTRACTPROCESSED-MONTH] Skipping older upload for contract ${contractId} (current ts: ${contractRow.timestamp} <= latest ts: ${existing.contractRow.timestamp})`);
                    }
                } else if (!entity) {
                    console.log(`⚠️ [CONTRACTPROCESSED-MONTH] Upload doc exists but no entity data for ${batchId} (entityBatchId: ${entityBatchId})`);
                }
            }
            ...
            console.log(`🔍 [CONTRACTPROCESSED-MONTH] Processing ${latestContractByContractId.size} latest contract uploads...`);
            ...
            console.log(`🔍 [CONTRACTPROCESSED-MONTH] Adding "all" status and unique contract ID metrics...`);
            ...
            for (const [countryStatus, count] of allByCountryStatus) {
                ...
                console.log(`📊 [CONTRACTPROCESSED-MONTH] All contract statuses for ${country}: ${status} = ${count}`);
            }
            ...
            for (const [status, count] of allByStatus) {
                ...
                console.log(`📊 [CONTRACTPROCESSED-MONTH] All contract statuses total: ${status} = ${count}`);
            }
            ...
            for (const [country, count] of uniqueByCountry) {
                ...
                console.log(`📊 [CONTRACTPROCESSED-MONTH] Unique contracts (all statuses): completed ${country} = ${count} unique contract IDs`);
            }

            // Add total unique "all" metric
            const allTotalKey = `completed_unique|total|all|0`;
            agg.set(allTotalKey, uniqueContractIdsTotal.size);
            console.log(`📊 [CONTRACTPROCESSED-MONTH] Unique contracts (all statuses): completed total = ${uniqueContractIdsTotal.size} unique contract IDs`);
            ...
            console.log(`✅ [CONTRACTPROCESSED-MONTH] Final aggregation has ${finalAgg.size} entries`);

            // Generate cumulative metrics (sum across all attempts) - ONLY THESE
            const cumulativeMetrics = generateProcessedCumulativeMetricsFromData(finalAgg, "month");
            console.log(`✅ [CONTRACTPROCESSED-MONTH] Generated ${cumulativeMetrics.length} cumulative metrics`);

            console.log(`✅ [CONTRACTPROCESSED-MONTH] Query completed successfully. Returning ${cumulativeMetrics.length} metrics`);

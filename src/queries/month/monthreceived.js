// src/queries/month/monthreceived.js
// All contract received queries for month
...
export const queries = [
    {
        name: "contractreceived_month",
        metricBase: "custom.dashboard.contractreceived.month.by_status",
        run: async (containers, win) => {
            console.log(`🔍 [CONTRACTRECEIVED-MONTH] Starting query for time window: ${win.startSec} to ${win.endSec}`);
            ...
            console.log(`🔍 [CONTRACTRECEIVED-MONTH] Querying contract table for upload documents...`);
            const { resources: uploadDocs } = await contractC.items.query(qUploadDocs).fetchAll();
            console.log(`✅ [CONTRACTRECEIVED-MONTH] Found ${uploadDocs?.length || 0} upload documents in time window`);

            if (!uploadDocs || uploadDocs.length === 0) {
                console.log(`⚠️ [CONTRACTRECEIVED-MONTH] No upload documents found, returning zero metrics`);
                return generateCumulativeMetricsFromData(new Map(), "month");
            }
            ...
            const cleanUploadDocs = uploadDocs.filter((d, i) => {
                ...
                if (!ok) {
                    console.warn(`[CONTRACTRECEIVED-MONTH] Skipping upload doc at index ${i} due to invalid batchId:`, {
            ...
            if (cleanUploadDocs.length === 0) {
                console.log(`⚠️ [CONTRACTRECEIVED-MONTH] No valid upload docs with batchId; returning zero metrics`);
                return generateCumulativeMetricsFromData(new Map(), "month");
            }

            const batchIds = cleanUploadDocs.map(d => toEntityBatchId(d.batchId)).filter(Boolean);
            console.log(`🔍 [CONTRACTRECEIVED-MONTH] Processing ${batchIds.length} batchIds for received analysis...`);
            ...
            for (let i = 0; i < batchIds.length; i += chunkSize) {
                ...
                console.log(`🔍 [CONTRACTRECEIVED-MONTH] Looking up entity data for chunk ${Math.floor(i/chunkSize) + 1}...`);
                ...
                for (const entity of (entities || [])) {
                    ...
                    console.log(`✅ [CONTRACTRECEIVED-MONTH] Entity data for ${entity.batchId}: country=${entity.countryCode}, status=${entity.contractStatus}, contractId=${entity.contractId}`);
                }
            }

            console.log(`✅ [CONTRACTRECEIVED-MONTH] Entity data lookup complete. Found ${entityData.size} entities`);

            // Step 3: Build a map of contractId -> most recent upload document
            console.log(`🔍 [CONTRACTRECEIVED-MONTH] Finding most recent upload for each contract...`);
            ...
            for (const uploadDoc of cleanUploadDocs) {
                ...
                if (!entityBatchId) {
                    console.warn(`[CONTRACTRECEIVED-MONTH] Derived empty entityBatchId from batchId='${batchId}', skipping`);
                    continue;
                }
                ...
                if (entity && entity.contractId) {
                    ...
                    if (!existing || uploadDoc.timestamp > existing.uploadDoc.timestamp) {
                        ...
                        if (existing) {
                            console.log(`� [CONTRACTRECEIVED-MONTH] Replaced older upload for contract ${contractId} (old ts: ${existing.uploadDoc.timestamp}, new ts: ${uploadDoc.timestamp})`);
                        }
                    } else {
                        console.log(`⏭️ [CONTRACTRECEIVED-MONTH] Skipping older upload for contract ${contractId} (current ts: ${uploadDoc.timestamp} <= latest ts: ${existing.uploadDoc.timestamp})`);
                    }
                } else if (!entity) {
                    console.log(`⚠️ [CONTRACTRECEIVED-MONTH] Upload doc exists but no entity data for ${batchId} (entityBatchId: ${entityBatchId})`);
                }
            }

            console.log(`✅ [CONTRACTRECEIVED-MONTH] Found ${latestUploadByContractId.size} unique contracts with their latest uploads`);
            ...
            console.log(`🔍 [CONTRACTRECEIVED-MONTH] Processing ${latestUploadByContractId.size} latest contract uploads...`);
            ...
            for (const [contractId, { uploadDoc, entity }] of latestUploadByContractId) {
                ...
                console.log(`📊 [CONTRACTRECEIVED-MONTH] Processing latest upload for contract ${contractId}: ${batchId} -> country=${country}, contractStatus=${cs}, attempts=${attempts}, timestamp=${uploadDoc.timestamp}`);
                ...
                console.log(`✅ [CONTRACTRECEIVED-MONTH] Successfully received: ${batchId} -> ${statusLabel}|${country}|${cs}|0 (attempts=${attempts}, status=${status})`);
            ...
            console.log(`🔍 [CONTRACTRECEIVED-MONTH] Adding unique contract ID metrics...`);
            ...
            for (const [country, count] of uniqueByCountry) {
                ...
                console.log(`📊 [CONTRACTRECEIVED-MONTH] Unique contracts (all statuses): successfully_received ${country} = ${count} unique contract IDs`);
            }
            ...
            for (const [country, count] of allByCountry) {
                ...
                console.log(`📊 [CONTRACTRECEIVED-MONTH] All contract statuses for ${country}: successfully_received = ${count}`);
            }
            ...
            console.log(`📊 [CONTRACTRECEIVED-MONTH] All contract statuses total: successfully_received = ${allTotalCount}`);
            const allTotalKey = `successfully_received_unique|total|all|0`;
            agg.set(allTotalKey, uniqueContractIdsTotal.size);
            console.log(`📊 [CONTRACTRECEIVED-MONTH] Unique contracts (all statuses): successfully_received total = ${uniqueContractIdsTotal.size} unique contract IDs`);
            ...
            console.log(`✅ [CONTRACTRECEIVED-MONTH] Final aggregation has ${finalAgg.size} entries`);

            // Generate cumulative metrics (sum across all attempts) - ONLY THESE
            const cumulativeMetrics = generateCumulativeMetricsFromData(finalAgg, "month");
            console.log(`✅ [CONTRACTRECEIVED-MONTH] Generated ${cumulativeMetrics.length} cumulative metrics`);

            console.log(`✅ [CONTRACTRECEIVED-MONTH] Query completed successfully. Returning ${cumulativeMetrics.length} metrics`);
            return cumulativeMetrics;
        }
    },
    {
        name: "contractreceived_subdomains_month",
        metricBase: "custom.dashboard.contractreceived.month.subdomains",
        run: async (containers, win) => {
            console.log(`🔍 [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Starting query for time window: ${win.startSec} to ${win.endSec}`);

            const entitiesC = containers.contractEntities;

            // Step 1: Find all unique contractIds in month's time window
            const qContractIds = {
                query: `
            ...
            console.log(`🔍 [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Querying for unique contractIds...`);
            const { resources: contractIds } = await entitiesC.items.query(qContractIds).fetchAll();
            console.log(`✅ [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Found ${contractIds?.length || 0} unique contractIds in time window`);

            if (!contractIds || contractIds.length === 0) {
                console.log(`⚠️ [CONTRACTRECEIVED-SUBDOMAINS-MONTH] No contractIds found, returning zero metrics`);
                return generateCumulativeMetricsFromData(new Map(), "month");
            }
            ...
            console.log(`🔍 [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Analyzing subDomains for ${contractIds.length} contracts...`);
            ...
            for (const contractIdRow of contractIds) {
                ...
                console.log(`\n📋 [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Analyzing contractId: ${contractId}`);
                ...
                if (subDomainRows && subDomainRows.length > 0) {
                    ...
                    for (const [subDomain, count] of subDomainCounts) {
                        const k = `${subDomain}|${country}|${contractStatus}|month`;
                        agg.set(k, count);
                    }
                }
            }
            ...
            const finalAgg = new Map([...agg, ...totalAgg]);
            console.log(`✅ [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Final aggregation has ${finalAgg.size} entries`);
            ...
            console.log(`✅ [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Generated ${results.length} metrics`);
            console.log(`✅ [CONTRACTRECEIVED-SUBDOMAINS-MONTH] Query completed successfully. Returning ${results.length} metrics`);
            return results;
        }
    }
];

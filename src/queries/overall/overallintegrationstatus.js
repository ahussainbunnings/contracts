// src/que        // Query for ContractCosmosDataModel documents
        const integrationQuery = {
            query: `
                ...
                ORDER BY c._ts DESC
            `
        };all/integrationstatus.js
export const name = "integrationstatus_overall";
export const metricBase = "custom.dashboard.integrationstatus.overall";

export async function run(containers, options = {}) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`🔍 [INTEGRATIONSTATUS-OVERALL] Starting query...`);
    console.log(`${'='.repeat(80)}`);

    try {
        // Query for all ContractCosmosDataModel documents to get integration status
        const integrationQuery = {
            query: `
                SELECT c.id, c.contractId, c.integrationStatus, c.integrationComment, c.contractStatus, c._ts
                FROM c 
                WHERE c.documentType = "ContractCosmosDataModel"
                ORDER BY c._ts DESC
            `
        ...
        console.log(`✅ [INTEGRATIONSTATUS-OVERALL] Found ${result.resources.length} contracts with integration status`);
        ...
        console.log(`🔍 [INTEGRATIONSTATUS-OVERALL] After deduplication: ${contractMap.size} unique contracts`);
        ...
        if (contractIds.length > 0) {
            console.log(`🔍 [INTEGRATIONSTATUS-OVERALL] Fetching country codes for ${contractIds.length} contracts...`);
            ...
            console.log(`✅ [INTEGRATIONSTATUS-OVERALL] Found country codes for ${countryMap.size} contracts`);
        }
        ...
        console.log(`\n✅ [INTEGRATIONSTATUS-OVERALL] Generated ${metrics.length} metrics`);

        return metrics;

    } catch (error) {
        console.error(`❌ [INTEGRATIONSTATUS-OVERALL] Error:`, error.message);

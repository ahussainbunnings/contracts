// src/queries/today/integrationstatus.js
export const name = "integrationstatus_today";
export const metricBase = "custom.dashboard.integrationstatus.today";

export async function run(containers, win) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`🔍 [INTEGRATIONSTATUS-TODAY] Starting query for time window: ${win.startSec} to ${win.endSec}`);
    ...
    try {
        ...
        console.log(`✅ [INTEGRATIONSTATUS-TODAY] Found ${result.resources.length} contracts in time window`);

        if (result.resources.length === 0) {
            console.log(`⚠️ [INTEGRATIONSTATUS-TODAY] No contracts found, returning zero metrics`);
            ...
            statuses.forEach(intStatus => {
                // Total
                metrics.push({
                    labels: { integration_status: intStatus, country: 'total', window: 'today' },
                    value: 0
                });
                ...
                ['au', 'nz'].forEach(country => {
                    metrics.push({
                        labels: { integration_status: intStatus, country: country, window: 'today' },
                        value: 0
                    });
                });
                ...
                contractStatuses.forEach(cStatus => {
                    metrics.push({
                        labels: { integration_status: intStatus, contract_status: cStatus, country: 'total', window: 'today' },
                        value: 0
                    });
                });
            });
        ...
        console.log(`🔍 [INTEGRATIONSTATUS-TODAY] After deduplication: ${contractMap.size} unique contracts`);
        ...
        if (contractIds.length > 0) {
            console.log(`🔍 [INTEGRATIONSTATUS-TODAY] Fetching country codes for ${contractIds.length} contracts...`);
            ...
            console.log(`✅ [INTEGRATIONSTATUS-TODAY] Found country codes for ${countryMap.size} contracts`);
        }
        ...
        contractMap.forEach((doc, contractId) => {
            ...
            console.log(`📊 [INTEGRATIONSTATUS-TODAY] Contract ${contractId}: integrationStatus=${integrationStatus}, contractStatus=${contractStatus}, country=${country}`);
        ...
        // Display summary
        console.log(`\n${'='.repeat(80)}`);
        console.log(`📊 INTEGRATION STATUS SUMMARY - TODAY`);
        console.log(`${'='.repeat(80)}`);
        ...
        console.log(`\n✅ [INTEGRATIONSTATUS-TODAY] Generated ${metrics.length} metrics`);

        return metrics;

    } catch (error) {
        console.error(`❌ [INTEGRATIONSTATUS-TODAY] Error:`, error.message);

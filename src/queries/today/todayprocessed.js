...
export const queries = [
    {
        ...
        run: async (containers, win) => {
            ...
            console.log(`🔍 [CONTRACTPROCESSED-TODAY] Finding most recent upload for each contract...`);
            ...
            for (const contractRow of cleanContractRows) {
                ...
                if (entity && entity.contractId) {
                    ...
                    if (!existing || contractRow.timestamp > existing.contractRow.timestamp) {
                        ...
                        if (existing) {
                            console.log(`🔄 [CONTRACTPROCESSED-TODAY] Replaced older upload for contract ${contractId} (old ts: ${existing.contractRow.timestamp}, new ts: ${contractRow.timestamp})`);
                        }
                    } else {
                        console.log(`⏭️ [CONTRACTPROCESSED-TODAY] Skipping older upload for contract ${contractId} (current ts: ${contractRow.timestamp} <= latest ts: ${existing.contractRow.timestamp})`);
                    }
                } else if (!entity) {
            ...
            console.log(`✅ [CONTRACTPROCESSED-TODAY] Found ${latestContractByContractId.size} unique contracts with their latest uploads`);
            ...
            if (contractIds.length > 0) {
                console.log(`🔍 [CONTRACTPROCESSED-TODAY] Checking integration status for ${contractIds.length} contracts...`);
                ...
                console.log(`✅ [CONTRACTPROCESSED-TODAY] Found integration status for ${integrationStatusMap.size} contracts`);
            }
            ...
            console.log(`🔍 [CONTRACTPROCESSED-TODAY] Processing ${latestContractByContractId.size} latest contract uploads...`);
            ...
            for (const [contractId, { contractRow, entity }] of latestContractByContractId) {
                ...
                console.log(`📊 [CONTRACTPROCESSED-TODAY] Processing latest upload for contract ${contractId}: ${batchId} -> status=${status}, country=${country}, contractStatus=${cs}, attempts=${attempts}, timestamp=${contractRow.timestamp}, integrationStatus=${integrationStatus}`);
                ...
                if (status === 'completed' && hasErrors) {
                    console.log(`⚠️ [CONTRACTPROCESSED-TODAY] Skipping contract ${contractId} - has integration errors (will be counted in integration status metrics only)`);
                    continue;
                }
            ...
            // Return ONLY the unique contracts completed metric (total across all countries)
            const uniqueMetric = {
                labels: {
                    status: "completed_unique",
                    contractstatus: "all",
                    country: "total",
                    window: "today"
                },
            ...
            console.log(`✅ [CONTRACTPROCESSED-TODAY] Returning 1 metric: ${uniqueContractIdsTotal.size} unique contracts completed`);
            return [uniqueMetric];
        }
    }
];

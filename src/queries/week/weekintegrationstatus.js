...
export async function run(containers, win) {
    console.log(`\n${'='.repeat(80)}`);
    console.log(`🔍 [INTEGRATIONSTATUS-WEEK] Starting query for time window: ${win.startSec} to ${win.endSec}`);
    ...
    try {
        ...
        if (result.resources.length === 0) {
            ...
            statuses.forEach(intStatus => {
                // Total
                metrics.push({
                    labels: { integration_status: intStatus, country: 'total', window: 'week' },
                    value: 0
                });
                ...
                ['au', 'nz'].forEach(country => {
                    metrics.push({
                        labels: { integration_status: intStatus, country: country, window: 'week' },
                        value: 0
                    });
                });
                ...
                contractStatuses.forEach(cStatus => {
                    metrics.push({
                        labels: { integration_status: intStatus, contract_status: cStatus, country: 'total', window: 'week' },
                        value: 0
                    });
                });
            });
        ...
        console.log(`🔍 [INTEGRATIONSTATUS-WEEK] After deduplication: ${contractMap.size} unique contracts`);
        ...
        if (contractIds.length > 0) {
            console.log(`🔍 [INTEGRATIONSTATUS-WEEK] Fetching country codes for ${contractIds.length} contracts...`);
            ...
            console.log(`✅ [INTEGRATIONSTATUS-WEEK] Found country codes for ${countryMap.size} contracts`);
        }
        ...
        contractMap.forEach((doc, contractId) => {
            ...
            console.log(`📊 [INTEGRATIONSTATUS-WEEK] Contract ${contractId}: integrationStatus=${integrationStatus}, contractStatus=${contractStatus}, country=${country}`);
